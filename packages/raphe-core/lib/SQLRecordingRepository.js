...
class SQLRecordingRepository {
  ...
  async create({ name, args, result }) {
    this.db.serialize(() => {
      ...
      this.db.run("INSERT INTO recordings (name, args, result) VALUES ($name, $args, $result);", params, err => {
        if (err) throw err;
      });
    });
  }
  ...
  async getAll(name) {
    return new Promise((resolve, reject) => {
      this.db.all("SELECT name, args, result FROM recordings WHERE name = ?", name, (err, rows) => {
        if (err) {
          reject(err);
        } else {
